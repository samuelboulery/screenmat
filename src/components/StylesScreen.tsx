...
export default function StylesScreen({
  ...
  return (
    <div className="stage-glow absolute inset-x-0 top-[58px] bottom-0 grid grid-cols-[236px_1fr_620px] overflow-hidden">
      <aside className="flex flex-col gap-2 overflow-y-auto border-r border-white/5 p-[22px_18px]">
        <MonoLabel>Saved — {styles.length}</MonoLabel>
        <div className="space-y-1">
          {styles.map((style) => (
            <Row key={style.id} active={style.id === activeId} onClick={() => onSelect(style.id)}>
              <span className="h-[19px] w-[26px] shrink-0 rounded-[4px] border border-hairline bg-sunken" />
              <span className="t-ui truncate">{style.name}</span>
            </Row>
          ))}
        </div>
        <DashedTile onClick={onImport} className="mt-auto h-11 shrink-0 font-mono text-[10px]">
          Import .json
        </DashedTile>
      </aside>

      <div className="flex flex-col gap-[26px] overflow-y-auto p-[26px_28px]">
        {active ? (
          <>
            <input
              value={active.name}
                ...
                <div className="grid grid-cols-3 gap-1.5">
                  {WATERMARK_POSITIONS.map((position) => (
                    <button
                      ...
                      className={`h-[30px] w-11 rounded-[7px] border transition-colors duration-140 ${
                ...
                {palette &&
                  [palette.base, ...palette.accents].map((color) => (
                    <span
                      ...
                      className="h-10 w-[54px] rounded-[9px] border border-white/10"
                    />
                  ))}
                <DashedTile className="h-10 w-[54px] rounded-[9px]" title="Add a color" disabled>
                  +
                </DashedTile>
              </div>
            </Section>
            ...
            <Section title="Locked in this style">
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  <div
                    key={label}
                    className="flex items-center justify-between rounded-[10px] border border-white/[.06] bg-white/[.03] px-3.5 py-3"
                  >
      ...
      <aside className="flex flex-col gap-4 overflow-y-auto border-l border-white/5 p-[26px_28px]">
        <MonoLabel>Live preview</MonoLabel>
        <div className="relative h-[308px] w-full overflow-hidden rounded-[14px] border border-hairline bg-sunken">
        ...
        <div className="flex gap-3">
          {shots.slice(0, 3).map((shot) => (
            <span
              key={shot.id}
              className="h-[62px] w-24 overflow-hidden rounded-lg border border-hairline bg-sunken"
            >
